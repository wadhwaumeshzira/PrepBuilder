 ...
 * @description securely delete a user account, requires password confirmation
 * @access private
 */
async function deleteAccountController(req, res) {
    ...
    const isPasswordValid = await bcrypt.compare(password, user.password);
    if (!isPasswordValid) {
        return res.status(400).json({ message: "Incorrect password. Deletion failed." });
    }

    // Cascade Delete: Destroy all interview reports created by this user
    await interviewReportModel.deleteMany({ user: req.user.id });

    // Delete the user record
    await userModel.findByIdAndDelete(req.user.id);

    // Invalidate session (blacklist token and clear cookie)
    const token = req.cookies.token;
    if (token) {
        await tokenBlacklistModel.create({ token });
    }
